...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    install -Dm755 bin/atlantic  "$out/bin/.atlantic-wrapped"
    install -Dm755 bin/atlanticd "$out/bin/.atlanticd-wrapped"
    runHook postInstall
  '';
  postFixup = ''
    for bin in atlantic atlanticd; do
      makeWrapper "$out/bin/.$bin-wrapped" "$out/bin/$bin" \
        "''${qtWrapperArgs[@]}" \
        --prefix QML2_IMPORT_PATH : "${qmlImportPath}" \
        --prefix QT_PLUGIN_PATH : "${qtPluginPath}" \
        --set ATLANTIC_DIR "$out/share/${finalAttrs.pname}" \
        --set ATLANTIC_VERSION "${finalAttrs.version}" \
  ...
  meta = with lib; {
    ...
    homepage = "https://github.com/kurbaga3131/atlantic";
    license = licenses.mit;
    platforms = platforms.linux;
    mainProgram = "atlantic";
  };
})
