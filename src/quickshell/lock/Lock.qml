...
Scope {
        ...
        command: [
            ...
            root.isNiri
                ...
                : (root.isSway
                    ...
                    : "layout=$(LC_ALL=C hyprctl devices -j 2>/dev/null | jq -r '(.keyboards[] | select(.main == true) | .active_keymap) // .keyboards[0].active_keymap // empty' | head -n1); [[ -z \"$layout\" || \"$layout\" == \"null\" ]] && layout=\"TR\"; echo \"${layout:0:2}\" | tr '[:lower:]' '[:upper:]'")
        ]
    ...
    Process {
        id: suspendProcess
        command: ["bash", Caching.atlanticDir + "/scripts/system/suspend.sh"]
        onExited: {
            SystemInfo.fetch();
            root.updateDeInfo();
    ...
    Process {
        id: poweroffProcess
        command: ["bash", Caching.atlanticDir + "/scripts/system/poweroff.sh"]
    }

    Process {
        id: reloadProcess
        command: ["bash", Caching.atlanticDir + "/scripts/system/reboot.sh"]
    }
