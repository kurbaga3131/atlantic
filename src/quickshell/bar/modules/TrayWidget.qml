...
Rectangle {
    ...
    readonly property int itemCount: {
        ...
        if (arr && arr.length !== undefined) {
            for (let i = 0; i < arr.length; i++) {
                if (arr[i] && !trayWidgetRoot.isItemIgnored(arr[i])) c++;
            }
            return c;
        }
        if (list.count !== undefined && typeof list.get === "function") {
            for (let j = 0; j < list.count; j++) {
                let it = list.get(j);
                if (it && !trayWidgetRoot.isItemIgnored(it)) c++;
            }
            return c;
        }
    ...
    Row {
        ...
        Repeater {
            ...
            onCountChanged: {
                trayWidgetRoot.forceCountUpdate++;
                if (count === 0 || trayWidgetRoot.itemCount === 0) {
                    TrayMenuController.hide();
                }
            }
            ...
            delegate: Image {
                id: trayIcon
                readonly property bool isBlacklisted: trayWidgetRoot.isItemIgnored(modelData)
                visible: !isBlacklisted
                width: isBlacklisted ? 0 : trayWidgetRoot.iconSize
                height: isBlacklisted ? 0 : trayWidgetRoot.iconSize
                ...
                opacity: isBlacklisted ? 0.0 : (initAnimTrigger ? (isHovered ? 1.0 : (trayWidgetRoot.isCompact ? 0.9 : 0.8)) : 0.0)
                ...
                Component.onDestruction: {
                    ...
                    if (isDc && typeof Quickshell !== "undefined") {
                        Quickshell.execDetached(["bash", "-c", "sleep 0.8; pkill -9 -fi discord 2>/dev/null; pkill -9 -fi vesktop 2>/dev/null; rm -f $HOME/.config/discord/SingletonLock $HOME/.config/discord/SingletonSocket 2>/dev/null"]);
                    }
                }
