...
Item {
    ...
    Process {
        ...
        running: false
        command: {
            let assetsDir = Caching.atlanticDir ? (Caching.atlanticDir + "/assets/widgets") : "";
            let userDir = Caching.stateDir ? (Caching.stateDir + "/widgets/presets") : (Caching.home + "/.local/state/atlantic/widgets/presets");
            let pyScript =
    ...
    function deleteCustomPreset(presetName) {
        ...
        let userPath = Caching.stateDir ? (Caching.stateDir + "/widgets/presets") : (Caching.home + "/.local/state/atlantic/widgets/presets");
    ...
    function openRedactor(mon) {
        let runnerTarget = Caching.atlanticDir ? (Caching.atlanticDir + "/quickshell/Runner.qml") : "";
        let redactorTarget = Caching.widgetRedactor || (Caching.atlanticDir ? Caching.atlanticDir + "/quickshell/widgets/WidgetRedactor.qml" : Caching.mainQml);
        let launchCmd = "{ mkdir -p '" + Caching.runDir + "' && printf '%s' '" + mon + "' > '" + Caching.runDir + "/redactor_target_monitor' && QS_WIDGET_MONITOR='" + mon + "' ATLANTIC_TARGET_FILE='" + redactorTarget + "' quickshell -p '" + runnerTarget + "'; } >> /tmp/redactor_debug.log 2>&1";
