...
Item {
    ...
    FileView {
        ...
        path: (typeof Caching !== "undefined" && Caching.stateDir ? Caching.stateDir : ((Quickshell.env("HOME") ?? "") + "/.local/state/atlantic")) + "/qs_matugen_colors.json"
        watchChanges: true
        onFileChanged: reload()
        onLoaded: {
            try {
    ...
    Process {
        ...
        running: false
        command: {
            let assetsPath = Caching.atlanticDir ? (Caching.atlanticDir + "/assets/themes") : "";
            let userPath = Caching.stateDir ? (Caching.stateDir + "/themes") : (Caching.home + "/.local/state/atlantic/themes");
    ...
    function installFont(filePath, fileName) {
        ...
        let userFontsPath = Caching.stateDir ? (Caching.stateDir + "/fonts") : (Caching.home + "/.local/state/atlantic/fonts");
        let escapeBash = function(str) { return String(str).replace(/(["\\$`])/g, '\\$1'); };
            ...
            Item {
                ...
                RowLayout {
                    ...
                    ClickButton {
                        ...
                        onClicked: {
                            Quickshell.execDetached(["bash", Caching.atlanticDir + "/scripts/qs_manager.sh", "toggle", "wallpaper"]);
                        }
                    }
