...
Item {
    ...
    function saveNotifiedVersion(ver) {
        if (typeof Caching === "undefined" || !Caching.atlanticDir) return;
        let stateDir = Caching.getStateDir();
        Quickshell.execDetached([
            "python3",
            Caching.atlanticDir + "/scripts/updater.py",
            "--state-dir",
            stateDir,
            "--save-notified",
            ver
    ...
    function sendNotification() {
        let serpDir = (typeof Caching !== "undefined" && Caching.atlanticDir) ? Caching.atlanticDir : "";
    ...
    FileView {
        ...
        path: (typeof Caching !== "undefined" && Caching.atlanticDir ? Caching.atlanticDir : "") + "/version.txt"
        onFileChanged: {
    ...
    FileView {
        ...
        onLoaded: {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.indexOf("ATLANTIC_VERSION=") === 0) {
                    let v = line.substring("ATLANTIC_VERSION=".length).replace(/["']/g, "").trim();
                    if (v) {
                        root.stateVersion = v;
                        root.syncLocalVersion();
                    }
    ...
    Process {
        ...
        command: [
            "python3",
            (typeof Caching !== "undefined" ? Caching.atlanticDir : "") + "/scripts/updater.py",
            "--state-dir",
