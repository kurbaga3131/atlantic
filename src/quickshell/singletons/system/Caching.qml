...
QtObject {
    ...
    readonly property string atlanticDir: Quickshell.env("ATLANTIC_DIR")
    ...
    readonly property string cacheDir: Quickshell.env("QS_CACHE_DIR") ? Quickshell.env("QS_CACHE_DIR") : (home + "/.cache/atlantic")
    readonly property string stateDir: Quickshell.env("QS_STATE_DIR") ? Quickshell.env("QS_STATE_DIR") : (home + "/.local/state/atlantic")
    readonly property string runDir: Quickshell.env("QS_RUN_DIR") ? Quickshell.env("QS_RUN_DIR") : ((xdgRuntimeDir !== "" ? xdgRuntimeDir : "/tmp") + "/atlantic")
    ...
    function getCacheDir(widgetName) {
        if (!widgetName || widgetName === "atlantic" || cacheDir.endsWith("/" + widgetName)) {
    ...
    function getStateDir(widgetName) {
        if (!widgetName || widgetName === "atlantic" || stateDir.endsWith("/" + widgetName)) {
    ...
    function getRunDir(widgetName) {
        if (!widgetName || widgetName === "atlantic" || runDir.endsWith("/" + widgetName)) {
    ...
    function getLogDir(widgetName) {
        if (!widgetName || widgetName === "atlantic" || logDir.endsWith("/" + widgetName)) {
