...
Item {
    ...
    property string matugenBaseDir: {
        ...
        if (typeof Caching !== "undefined" && Caching.qsDir) {
            dir = Caching.atlanticDir + "/assets/matugen";
        } else if (typeof Caching !== "undefined" && Caching.atlanticDir) {
            dir = Caching.atlanticDir + "/src/assets/matugen";
        } else {
            dir = Quickshell.env("HOME") + "/.local/share/atlantic/src/assets/matugen";
        }
        return dir;
    }
    ...
    function _startStaticGenerate(colorsObj, rawJson, selectedMode) {
        ...
        let script =
            "STATE_DIR=\"$HOME/.local/state/atlantic\"; " +
        ...
        onExited: (exitCode) => {
            ...
            if (success) {
                ...
                if (matugenProcess.reqType === "image") {
                    let stateDir = (typeof Caching !== "undefined" && Caching.stateDir) ? Caching.stateDir : (Quickshell.env("HOME") + "/.local/state/atlantic");
