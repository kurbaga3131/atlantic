...
STATE_DIR="${QS_STATE_DIR:-$HOME/.local/state/atlantic}"
...
check_status() {
    ...
    START_QML=""
    if [ -f "$ATLANTIC_DIR/quickshell/serp/Start.qml" ]; then
        START_QML="$ATLANTIC_DIR/quickshell/serp/Start.qml"
    elif [ -f "$ATLANTIC_DIR/quickshell/Start.qml" ]; then
        START_QML="$ATLANTIC_DIR/quickshell/Start.qml"
    else
        START_QML="$(find "$ATLANTIC_DIR/quickshell" -type f -name "Start.qml" 2>/dev/null | head -n 1)"
    fi
...
open_guide() {
    local script_path="$ATLANTIC_DIR/scripts/qs_manager.sh"
