...
if [ -z "$ATLANTIC_DIR" ]; then
    if [ -d "$(dirname "$(dirname "$MODULE_DIR")")/src" ]; then
        export ATLANTIC_DIR="$(dirname "$(dirname "$MODULE_DIR")")/src"
    fi
fi

if [ -n "$ATLANTIC_DIR" ] && [ -f "$ATLANTIC_DIR/scripts/caching.sh" ]; then
    source "$ATLANTIC_DIR/scripts/caching.sh"
fi

STATE_DIR="${QS_STATE_DIR:-${XDG_STATE_HOME:-$HOME/.local/state}/atlantic}"
...
get_installed_commit() {
    if [ -f "$VERSION_FILE" ]; then
        awk -F= '/^ATLANTIC_COMMIT=/{gsub(/"/, "", $2); print $2}' "$VERSION_FILE"
    fi
}

get_target_version() {
    local repo_root="$1"
    local repo_slug="${2:-"${REPO_SLUG:-"kurbaga3131/atlantic"}"}"
    local target_ver=""
    ...
    if [[ -z "$target_ver" || "$target_ver" == "null" ]]; then
        if command -v curl &>/dev/null; then
            target_ver=$(curl -s "https://raw.githubusercontent.com/${repo_slug}/main/version.txt" 2>/dev/null | xargs)
        fi
    fi
...
get_target_commit() {
    ...
    if [[ -z "$target_commit" || "$target_commit" == "null" ]]; then
        if command -v curl &>/dev/null && command -v jq &>/dev/null; then
            target_commit=$(curl -s "https://api.github.com/repos/${repo_slug}/commits/main" 2>/dev/null | jq -r '.sha[:7] // empty')
        fi
    fi
...
write_version_state() {
    ...
    cat <<EOF > "$tmp_file"
ATLANTIC_VERSION="$version"
ATLANTIC_COMMIT="$commit"
TELEMETRY_ID="$tel_id"
