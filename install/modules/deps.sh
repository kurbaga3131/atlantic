...
REQUIRED_PKGS=(
    ...
    "steam" "lib32-mesa" "vulkan-radeon" "lib32-vulkan-radeon" "lib32-vulkan-icd-loader" "ttf-liberation"
...
bootstrap_installer_deps() {
    ...
    if ! command -v yay &>/dev/null && ! command -v paru &>/dev/null; then
        local cache_build="${XDG_CACHE_HOME:-"$HOME/.cache"}/atlantic-yay-bin"
        rm -rf "$cache_build"
        mkdir -p "$cache_build"
...
install_fonts() {
    ...
    if [ ! -d "$target_fonts_dir" ] || [ -z "$(ls -A "$target_fonts_dir" 2>/dev/null | grep -i "\.ttf")" ]; then
        local font_cache="${XDG_CACHE_HOME:-"$HOME/.cache"}/atlantic-fonts"
