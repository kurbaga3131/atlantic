...
init_compositor_detection() {
    ...
    elif [ -f "$HOME/.local/state/atlantic/version" ]; then
        local saved_comps
        saved_comps=$(awk -F= '/^SELECTED_COMPOSITORS=/{gsub(/"/, "", $2); print $2}' "$HOME/.local/state/atlantic/version" 2>/dev/null || true)
    ...
    if [ ${#SELECTED_COMPOSITORS[@]} -eq 0 ]; then
        if [ ${#installed[@]} -gt 1 ]; then
            ...
            SELECTED_COMPOSITORS=("hyprland" "niri" "sway")
...
EOF
    ...
    local OSC8_TW=$'\e]8;;https://twitter.com/kurbaga3131x\a'
    local OSC8_RD=$'\e]8;;https://reddit.com/u/kurbaga31311\a'
    local OSC8_TG=$'\e]8;;https://t.me/stewart_github\a'
    local OSC8_KF=$'\e]8;;https://ko-fi.com/kurbaga3131\a'
    local OSC8_END=$'\e]8;;\a'

    printf "\033[K%s--------------------------------------------------------------------------------%s\n" "$C_BLUE" "$RESET"
    printf "\033[K%sDiscord:%s https://discord.gg/vcJxzMPY78\n" "$C_BLUE" "$RESET"
    printf "\033[K%s--------------------------------------------------------------------------------%s\n" "$C_BLUE" "$RESET"
    printf "\033[K%s $(t "installer.ui.user")%s %-25s | %s$(t "installer.ui.os")%s %s\n" "$BOLD" "$RESET" "$USER_NAME" "$BOLD" "$RESET" "$OS_NAME"
    printf "\033[K%s $(t "installer.ui.cpu")%s  %s\n" "$BOLD" "$RESET" "$CPU_INFO"
    printf "\033[K%s--------------------------------------------------------------------------------%s\n" "$C_BLUE" "$RESET"
    if [[ "$INSTALL_STATE" == "current" ]]; then
        printf "\033[K%s $(t "installer.ui.version_to_update")%s %s\n" "$BOLD" "$RESET" "$TARGET_VERSION"
    else
        printf "\033[K%s $(t "installer.ui.target_version")%s %-10s | %s$(t "installer.ui.install_mode")%s %s\n" "$BOLD" "$RESET" "$TARGET_VERSION" "$BOLD" "$RESET" "$INSTALL_STATE"
    fi
    printf "\033[K%s================================================================================%s\n\n" "$C_BLUE" "$RESET"
}
...
run_updater_ui() {
    ...
    while true; do
        ...
        while true; do
            local items=(
                "1. ${BOLD}${C_GREEN}$(t "installer.ui.menu_update")${RESET}"
                "2. ${DIM}$(t "installer.ui.menu_exit")${RESET}"
            )
            ...
            key=$(read_key)
            case "$key" in
                UP|[kK]) cursor=$(( cursor > 0 ? cursor - 1 : ${#items[@]} - 1 )) ;;
                DOWN|[jJ]) cursor=$(( cursor < ${#items[@]} - 1 ? cursor + 1 : 0 )) ;;
                "1") IS_REINSTALL=false; cleanup_terminal; return 0 ;;
                "2"|ESC|[qQ]) cleanup_terminal; clear; exit 0 ;;
                ENTER|SPACE)
                    if [[ "${items[$cursor]}" == *"1."* ]]; then
                        IS_REINSTALL=false
                        cleanup_terminal
                        return 0
                    elif [[ "${items[$cursor]}" == *"2."* ]]; then
                        cleanup_terminal
                        clear
                        exit 0
                    fi
...
draw_completion_screen() {
    ...
    clear
    printf "%s%s" "$BOLD" "$C_CYAN"
    if [[ "$INSTALL_STATE" == "current" && -z "$POST_INSTALL" ]]; then
        cat << "EOF"
 █████╗ ████████╗██╗      █████╗ ███╗   ██╗████████╗██╗ ██████╗    ██╗   ██╗██████╗     ███████╗██╗███╗   ██╗██╗███████╗██╗  ██╗███████╗██████╗ 
██╔══██╗╚══██╔══╝██║     ██╔══██╗████╗  ██║╚══██╔══╝██║██╔════╝    ██║   ██║██╔══██╗    ██╔════╝██║████╗  ██║██║██╔════╝██║  ██║██╔════╝██╔══██╗
███████║   ██║   ██║     ███████║██╔██╗ ██║   ██║   ██║██║         ██║   ██║██████╔╝    █████╗  ██║██╔██╗ ██║██║███████╗███████║█████╗  ██║  ██║
...
EOF
    ...
    printf "%s%s================================================================================%s\n" "$BOLD" "$C_BLUE" "$RESET"
    printf "%s%s %s:%s\n" "$BOLD" "$C_YELLOW" "$(t "installer.ui.socials")" "$RESET"
    printf " %sDiscord:%s https://discord.gg/vcJxzMPY78\n" "$C_BLUE" "$RESET"
    printf "%s%s================================================================================%s\n\n" "$BOLD" "$C_BLUE" "$RESET"
    
    if [[ "$INSTALL_STATE" == "current" && -z "$POST_INSTALL" ]]; then
        printf "%s%s%s\n" "$C_GREEN" "$(t "installer.ui.updated_success" "ver=$target_ver")" "$RESET"
    else
        printf "%s%s%s\n" "$C_GREEN" "$(t "installer.ui.installed_success" "ver=$target_ver")" "$RESET"
    fi
