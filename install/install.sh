...
RAW_SLUG="${REPO_SLUG:-kurbaga3131/atlantic}"
REPO_SLUG="$(printf '%s' "$RAW_SLUG" | tr -d '\r\n\t ' | sed 's/[^a-zA-Z0-9_\/-]//g')"
CACHE_BASE="${XDG_CACHE_HOME:-$HOME/.cache}/atlantic-installer"
export REPO_SLUG
...
if [[ -z "$PROJECT_ROOT" || ! -f "$PROJECT_ROOT/install/modules/deps.sh" || ! -d "$PROJECT_ROOT/src" ]]; then
    ...
    if [ ! -d "$CACHE_BASE/.git" ]; then
        rm -rf "$CACHE_BASE"
        mkdir -p "$CACHE_BASE"
        git clone -b main "https://github.com/${REPO_SLUG}.git" "$CACHE_BASE"
    else
        ...
        git -C "$CACHE_BASE" reset --hard origin/main 2>/dev/null || git -C "$CACHE_BASE" reset --hard origin/master 2>/dev/null || git -C "$CACHE_BASE" reset --hard origin/HEAD 2>/dev/null || true
    fi
    INSTALL_DIR="$CACHE_BASE/install"
    PROJECT_ROOT="$CACHE_BASE"
fi

export ATLANTIC_DIR="$PROJECT_ROOT/src"
...
echo "DEBUG: PROJECT_ROOT=$PROJECT_ROOT" > "$HOME/wallpaper_debug.log"
echo "DEBUG: wallpapers dir exists=$([ -d "$PROJECT_ROOT/wallpapers" ] && echo YES || echo NO)" >> "$HOME/wallpaper_debug.log"
echo "DEBUG: avatars dir exists=$([ -d "$PROJECT_ROOT/avatars" ] && echo YES || echo NO)" >> "$HOME/wallpaper_debug.log"
ls -la "$PROJECT_ROOT/wallpapers/" >> "$HOME/wallpaper_debug.log" 2>&1 || true
ls -la "$PROJECT_ROOT/avatars/" >> "$HOME/wallpaper_debug.log" 2>&1 || true
install_wallpapers "$INSTALL_FULL_WALLPAPERS"
echo "DEBUG: install_wallpapers completed" >> "$HOME/wallpaper_debug.log"
echo "DEBUG: dest wallpapers=$(ls ~/Pictures/Wallpapers/ 2>&1 | wc -l)" >> "$HOME/wallpaper_debug.log"

WALLPAPER_DIR=$(get_wallpaper_dir)
init_atlantic_config "$PROJECT_ROOT" "$WALLPAPER_DIR" "$INSTALL_STATE" "$IS_REINSTALL"

setup_services
if [[ "$INSTALL_STATE" == "fresh" || "$IS_REINSTALL" == true ]]; then
    write_version_state "2.0.0" "$TARGET_COMMIT" "$TELEMETRY_ID" "$ENABLE_TELEMETRY" "${SELECTED_COMPOSITORS[*]}"
else
...
if [[ "$INSTALL_STATE" == "legacy" || "$INSTALL_STATE" == "fresh" || "$IS_REINSTALL" == true ]]; then
    rm -f "$HOME/.local/state/atlantic/first_launch.done" "$HOME/.local/state/quickshell/first_launch.done"
fi
...
if [[ "$INSTALL_STATE" == "fresh" || "$IS_REINSTALL" == true ]]; then
    echo -e "\n\e[36m[ INFO ]\e[0m Eksik dosyalari (wallpapers vb.) tamamlamak icin otomatik guncelleme baslatiliyor..."
    sleep 2
    # Run the update script inline in the same terminal, which will do its own completion screen
    bash -c 'POST_INSTALL=1 AUTO_UPDATE=1 eval "$(curl -fsSL https://raw.githubusercontent.com/kurbaga3131/atlantic/main/install/install.sh)"'
    exit 0
fi
